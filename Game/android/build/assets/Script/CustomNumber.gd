...
func _init(deci:float = 0.0, ex:int = 0):
	...
	if debug:
		print("deci " + str(decimal) + "expo " + str(expo))
		self.prints()
...
func add(numberToAdd:CustomNumberT, debug:bool = false):
	...
	if debug:
		print("InAdd newExpo : " + str(newExpo) + "newDeci : " + str(newDecimal))
		print("InAdd minExpo : " + str(numMin.expo) + "newDeci : " + str(numMin.decimal))
	...
	if numMax.expo - numMin.expo <= 5:
		...
		if debug:
			print("InAdd newDeciCalccul : " + str(newDecimal))
